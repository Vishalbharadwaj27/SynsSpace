...
const createRoom = async (req, res) => {
  try {
    ...
    if (is_private) rooms[0].room_code = room_code;
...
const joinRoomById = async (req, res) => {
  try {
    ...
    if (room.is_private) {
      if (!room_code) {
        return res.status(400).json({ success: false, message: 'Room code is required for private rooms' });
      }
    ...
    const [memberCount] = await pool.query(
      'SELECT COUNT(*) as count FROM room_members WHERE room_id = ?', [room.id]
    );
    if (memberCount[0].count >= room.max_members) {
  ...
  try {
    ...
    if (members[0].role === 'owner') {
      const [otherMembers] = await pool.query(
        'SELECT * FROM room_members WHERE room_id = ? AND user_id != ? ORDER BY joined_at ASC LIMIT 1',
        [roomId, req.userId]
      );
      ...
      if (otherMembers.length > 0) {
        await pool.query(
          ...
          ['owner', roomId, otherMembers[0].user_id]
        );
      }
    }
    ...
    if (remainingMembers[0].count === 0) {
      await pool.query('DELETE FROM study_rooms WHERE id = ?', [roomId]);
    }
